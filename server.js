...
app.get("/apartments/:index/shower", function (req, res) {
  res.json(
    db.getData(
      `/houses[0]/apartments[${req.params.index}]/Hydractiva_shower/measurements`
    )
  )
})
...
app.get("/apartments/:index/kitchen-faucet", function (req, res) {
  res.json(
    db.getData(
      `/houses[0]/apartments[${req.params.index}]/Kitchen_optima_faucet/measurements`
    )
  )
})
...
app.get("/apartments/:index/optima-faucet", function (req, res) {
  res.json(
    db.getData(
      `/houses[0]/apartments[${req.params.index}]/Optima_faucet/measurements`
    )
  )
})
...
app.get("/apartments/:index/washing-machine", function (req, res) {
  res.json(
    db.getData(
      `/houses[0]/apartments[${req.params.index}]/Washing_machine/measurements`
    )
  )
})
...
app.get("/apartments/:index/dishwasher", function (req, res) {
  res.json(
    db.getData(
      `/houses[0]/apartments[${req.params.index}]/Dishwasher/measurements`
    )
  )
})

app.get("/apartments/:index/month-temp", function (req, res) {
  const { Hydractiva_shower, Kitchen_optima_faucet, Optima_faucet } = queryDb(
    req.params.index
  )
  ...
  ;[Hydractiva_shower, Kitchen_optima_faucet, Optima_faucet].forEach(
    ({ measurements }) =>
      measurements.forEach(({ TimeStamp, Temp }) => {
...
app.get("/apartments/:index/month", function (req, res) {
  ...
  ;[
    ...
    measurements.forEach(
      ({ TimeStamp, Consumption, FlowTime, Power_Consumption }) => {
        const date = new Date(TimeStamp)
        const month = getMonth(date)
...
app.get("/apartments/:index/water-usage", function (req, res) {
  ...
  ;[
    ...
    measurements.forEach(({ TimeStamp, Consumption }) => {
...
app.get("/apartments/:index/power-usage", function (req, res) {
  ...
  ;[
    ...
    measurements.forEach(({ TimeStamp, Power_Consumption }) => {
      const date = new Date(TimeStamp.replace("2020", "2021"))
...
const queryDb = (index) => db.getData(`/houses[0]/apartments[${index}]`)
...
const round = (result) => ({
  ...
  Power_Consumption: result.Power_Consumption.map((item) =>
